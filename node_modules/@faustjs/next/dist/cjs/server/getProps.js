...
const client_2 = require("../gqty/client");
...
async function getProps(context, { client, Page, props, }) {
    ...
    let authSnapshot;
    client.setAsRoot();
    if (!(0, isNil_1.default)(Page)) {
        const authClient = client.auth.client;
        const { cacheSnapshot: coreAuthSnapshot } = await authClient.prepareRender(async () => {
            const { cacheSnapshot: coreSnapshot } = await client.prepareReactRender(react_1.default.createElement(router_context_1.RouterContext.Provider
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            , { 
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                value: { query: { ...context.params } } },
                react_1.default.createElement(client_2.FaustContext.Provider, { value: { client } },
...
async function is404({ params }, { client }) {
    ...
    try {
        ...
        else if ((0, utils_1.hasPostSlug)(params)) {
            result = inlineResolved(() => {
                var _a;
                return (_a = query.post({
                    id: params.postSlug,
        ...
        else if ((0, utils_1.hasPostUri)(params)) {
            result = inlineResolved(() => {
                var _a;
                return (_a = query.post({
        ...
        else if ((0, utils_1.hasPageId)(params)) {
            result = inlineResolved(() => {
                var _a;
        ...
        else if ((0, utils_1.hasPageUri)(params)) {
            result = inlineResolved(() => {
                var _a;
                return (_a = query.page({
        ...
        else if ((0, utils_1.hasCategoryId)(params)) {
            result = inlineResolved(() => {
                var _a;
        ...
        else if ((0, utils_1.hasCategorySlug)(params)) {
            result = inlineResolved(() => {
                var _a;
                return (_a = query.category({
    ...
    if (result instanceof Promise) {
        entityExists = !(0, isNil_1.default)(await result);
    }
    else {
        entityExists = !(0, isNil_1.default)(result);
    }
    return !entityExists;
}
exports.is404 = is404;
...
async function getNextStaticProps(context, cfg) {
    ...
    const nextConfig = (0, config_1.config)();
    if ((0, lodash_1.isBoolean)(notFound) && notFound === true) {
        return {
            notFound,
        };
    ...
    if ((0, lodash_1.isObject)(pageProps.props)) {
        pageProps.revalidate = !(0, isNil_1.default)(revalidate)
            ? revalidate
