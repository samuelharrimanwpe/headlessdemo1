...
function createQueryFetcher(applyRequestContext) {
    return async function (query, variables) {
        const url = (0, config_1.getGqlUrl)();
        const headers = {
        ...
        if ((0, isFunction_1.default)(applyRequestContext)) {
...
function createAuthQueryFetcher(context, applyRequestContext) {
    const applyRequestContextFn = async (url, init) => {
        let token;
        if (!(0, isNil_1.default)(context)) {
            const { Cookies } = await Promise.resolve().then(() => __importStar(require('../server/auth/cookie')));
            const { OAuth } = await Promise.resolve().then(() => __importStar(require('../server/auth/token')));
...
function contextHasClient(context) {
    return ((0, isObject_1.default)(context) &&
        (0, isObject_1.default)(context.apiClient));
}
...
function getClient(clientConfig) {
    ...
    if ((0, isNil_1.default)(schema) || (0, isNil_1.default)(scalarsEnumsHash)) {
    ...
    const apiClient = {
        ...(0, gqty_1.createClient)({
            ...
            ...(0, omit_1.default)(clientConfig, 'context', 'applyRequestContext', 'authQueryFetcher', 'schema', 'scalarsEnumsHash'),
        }),
        auth: {
            ...(0, gqty_1.createClient)({
                schema,
    ...
    if ((0, isObject_1.default)(clientConfig.context)) {
